...
ready_table_spain = "/work/dlvr_just-renewability_F24/output/articles_sp_monthly_summary.csv"

ready_table_argentina = "/work/dlvr_just-renewability_F24/output/articles_arg_monthly_summary.csv"

#X-axis: Year-month and Y-axis: number of articles

def visualize(input_dataset, output_png, title):
    
    figure = pd.read_csv(input_dataset)
    figure = figure.sort_values("Year-Month")
    ...
    middle_bar = mpatches.Patch(color='royalblue', label='Filtered')
    bottom_bar = mpatches.Patch(color='lightblue', label='Used')
...
visualize(ready_table_spain, "/work/dlvr_just-renewability_F24/output/plots/articles_sp_timeline.png", "Spain articles chronological order" )
visualize(ready_table_argentina, "/work/dlvr_just-renewability_F24/output/plots/articles_arg_timeline.png", "Argentina articles chronological order")

####################TASK 3: 

excel_load_spain = pd.read_excel("output/article-filtering/articles_sp_meta_2024-08-22_renewable-energy_green-hydrogen_selection-nov24.xlsx")
excel_load_argentin = pd.read_excel("output/article-filtering/articles_arg_meta_2024-08-22_renewable-energy_green-hydrogen_revisadoLW_jun25.xlsx")

def task3(input_file):
    #Call previously defined function for filtering of the keyword column and grou
    compute_filtered_counts(input_file)
